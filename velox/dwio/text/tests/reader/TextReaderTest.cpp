...
public:
  void SetUp() override {
    ...
    schema_ =  ROW({"c0", "c1", "c2", "c3", "c4", "c5", "c6", "c7", "c8"},
          {BOOLEAN(),
           TINYINT(),
           SMALLINT(),
           INTEGER(),
           ...
           TIMESTAMP(),
           VARCHAR()});
    std::shared_ptr<exec::test::TempDirectoryPath> tempPath = exec::test::TempDirectoryPath::create();
    filePath_ = fs::path(fmt::format("{}/test_text_reader.txt", tempPath->getPath()));
    initFileData();
    initFileReader();
  }
 ...
 protected:
  ...
  void initFileReader() {
    ...
    auto fs = filesystems::getFileSystem(filePath_, std::make_shared<config::ConfigBase>(std::move(configMap)));
    ...
    auto readerFactory = dwio::common::getReaderFactory(dwio::common::FileFormat::TEXT);
...
TEST_F(TextReaderTest, TestReadData) {
  ...
  ASSERT_TRUE(v0->valueAt(0) == true && v0->valueAt(1) == true && v0->valueAt(2) == true);
  ASSERT_TRUE(v1->valueAt(0) == 1 && v1->valueAt(1) == 2 && v1->valueAt(2) == 3);
  ASSERT_TRUE(v2->valueAt(0) == 1 && v2->valueAt(1) == 2 && v2->valueAt(2) == 3);
  ASSERT_TRUE(v3->valueAt(0) == 1 && v3->valueAt(1) == 2 && v3->valueAt(2) == 3);
  ASSERT_TRUE(v4->valueAt(0) == 1 && v4->valueAt(1) == 2 && v4->valueAt(2) == 3);
  ASSERT_TRUE(v5->valueAt(30) == 1.1f && v5->valueAt(31) == kInf && v5->valueAt(32) == 3.1f);
  ASSERT_TRUE(v6->valueAt(0) == 1.1 && std::to_string(v6->valueAt(1)) == "nan" && v6->valueAt(2) == 3.1);
  for (size_t i = 0; i < 3; ++i) {
    auto t = v7->valueAt(i);
    ASSERT_TRUE(t.getSeconds() == i && t.getNanos() == i * 1'000'000);
  }
  ASSERT_TRUE(v8->valueAt(3069).str() == "hello" && v8->valueAt(3070).str() == "world" && v8->valueAt(3071).str() == "cpp");
}
