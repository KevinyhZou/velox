...
public:
    ...
    std::shared_ptr<MapState<uint32_t, int, RowVectorPtr, int>>getOrCreateMapState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<ListState<uint32_t, long, RowVectorPtr>> getOrCreateListState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<ValueState<uint32_t, long, RowVectorPtr>> getOrCreateValueState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<ValueState<uint32_t, TimeWindow, RowVectorPtr>> getOrCreateGroupValueState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<MapState<uint32_t, int, TimeWindow, TimeWindow>> getOrCreateGroupMapState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<MapState<uint32_t, int, uint32_t, RowVectorPtr>> getOrCreateRankMapState(StateDescriptor& stateDescriptor) override;

    std::shared_ptr<InternalTimerService<uint32_t, long>> createTimerService(Triggerable<uint32_t, long>* triggerable) override;
...
private:
    ...
    std::unordered_map<std::string, rocksdb::ColumnFamilyHandle*> stateColumnFamilies_;
};
