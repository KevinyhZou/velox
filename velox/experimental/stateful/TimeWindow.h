...
public:
    ...
    const RowVectorPtr getOutput(exec::Window* executor) {
        VELOX_CHECK(executor != nullptr);
        executor->addInput(input_);
        executor->noMoreInput();
...
class TimeWindow : public StatefulOperator {
public:
  TimeWindow(
  ...
  void close() override;
  void processWatermark(long timestamp, int index) override {}
