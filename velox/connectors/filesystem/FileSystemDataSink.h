...
public:
  FsWriterParameters(
      ...
      : hive::HiveWriterParameters(hive::HiveWriterParameters::UpdateMode::kAppend,
          partitionName, targetFileName, targetDirectory, writeFileName, writeDirectory) {}
};
...
private:
    ...
    FOLLY_ALWAYS_INLINE void checkRunning() const {
      VELOX_CHECK_EQ(stateString(state_), "RUNNING", "FileSystem data sink is not running");
    }
    ...
    FsWriterParameters getWriterParameters(const std::optional<std::string>& partition) const;
    ...
    std::shared_ptr<memory::MemoryPool> createWriterPool(const FsWriterId& writerId);

    const int64_t extractTimestampFromPartitionName(const std::string& partitionName);
};
