...
public:
    ...
    const std::vector<std::string> inputValues0 = {"2025-07-11", "14", "axxx11", "bxxx11"};
    const std::vector<std::string> inputValues1 = {"2025-07-11", "15", "axxx12", "bxxx12"};
    ...
    void SetUp() override {
        ...
        init();
        connector::registerConnectorFactory(std::make_shared<connector::filesystem::FileSystemConnectorFactory>());
        ...
        configMap[connector::filesystem::FileSystemWriteConfig::kFileNamePrefix] = fileNamePrefix;
        configMap[connector::filesystem::FileSystemWriteConfig::kFileNameSuffix] = fileNameSuffix;
        ...
        auto fileSystemConnector = connector::getConnectorFactory(
            connector::filesystem::FileSystemConnectorFactory::kFileSystemConnectorName
    ...
    void init() {
        ...
        sprintf(writePath, "%s%s%s%s", "file://", currentPath, "/", writeDir.data());
        dataPath = std::string(writePath,  strlen(currentPath) + writeDir.size() + 8);
    }
  ...
  const void insertData(const std::vector<std::string> & data, const size_t index, RowVectorPtr & inputRow) {
  ...
  const std::shared_ptr<connector::filesystem::FileSystemInsertTableHandle> createFsTableHandle(
    const std::vector<uint32_t>& partitionIndexes,
    const std::vector<std::string>& partitionKeys,
    const std::unordered_map<std::string, std::string>& tableParams = {}) {
    
    std::unordered_map<common::Subfield, std::unique_ptr<common::Filter>> subFieldFilters;
    return std::make_shared<connector::filesystem::FileSystemInsertTableHandle>("FilesystemWriter", 
        inputType,
        partitionIndexes,
        partitionKeys,
        tableParams
