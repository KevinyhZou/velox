...
class FileSystemConnectorTest : public FileSystemConnectorTestBase {
    FileSystemDataSink* createFileSystemSink(const std::vector<uint32_t>& partitionIndexes, const std::vector<std::string>&);
};

TEST_F(FileSystemConnectorTest, testConfig) {
    std::shared_ptr<connector::Connector> fsConnector = getConnector(fileSystemConnectorId);
    ASSERT_TRUE(fsConnector != nullptr);
    const std::shared_ptr<const config::ConfigBase>& connectorConfig = fsConnector->connectorConfig();
    const std::shared_ptr<FileSystemWriteConfig> writeConfig = std::make_shared<FileSystemWriteConfig>(connectorConfig);
    ASSERT_TRUE(writeConfig->exists(connector::filesystem::FileSystemWriteConfig::kPath));
    ASSERT_TRUE(writeConfig->exists(connector::filesystem::FileSystemWriteConfig::kFormat));
    ASSERT_TRUE(writeConfig->exists(connector::filesystem::FileSystemWriteConfig::kTaskId));
}

TEST_F(FileSystemConnectorTest, testWriteNonPartitionedTable) {
    std::shared_ptr<connector::Connector> fsConnector = getConnector(fileSystemConnectorId);
    std::unique_ptr<DataSink> dataSink = fsConnector->createDataSink(inputType,
        createFsTableHandle({}, {}),
        createQueryCtx().get(),
        CommitStrategy::kNoCommit);
    FileSystemDataSink * fsDataSink = reinterpret_cast<FileSystemDataSink *>(dataSink.get());
    ASSERT_TRUE(fsDataSink != nullptr);
    const RowVectorPtr inputData = createSingleInputRow();
    fsDataSink->appendData(inputData);
    const std::vector<std::shared_ptr<FsWriterInfo>> writerInfos = fsDataSink->getWriteInfos();
    ...
    const std::string writeFilePath = writerParams.writeDirectory() + "/" + writerParams.writeFileName();
    const std::string targetFilePath = writerParams.targetDirectory() + "/" + writerParams.targetFileName();
    auto fs_ = filesystems::getFileSystem(writeFilePath, fsConnector->connectorConfig());
    ...
    boost::algorithm::split(lineValues, line, boost::algorithm::is_any_of("\x01"));
...
TEST_F(FileSystemConnectorTest, testWritePartitionedTable) {
    ...
    for (size_t i = 0; i < writerInfos.size(); ++i) {
        ...
        const auto writerParams = writerInfo->writerParameters;
        const std::optional<std::string> partitionName = writerParams.partitionName();
        ASSERT_TRUE(partitionName.has_value());
        std::vector<std::string> partitionNames;
        boost::algorithm::split(partitionNames, partitionName.value(), boost::algorithm::is_any_of("/"));
        ASSERT_TRUE(partitionNames.size() == 2);
        for (size_t j = 0; j < partitionNames.size(); ++j) {
            std::vector<std::string> partitionKV;
            boost::algorithm::split(partitionKV, partitionNames[j], boost::algorithm::is_any_of("="));
        ...
        ASSERT_TRUE(writerParams.writeDirectory() == dataPath + "/" + partitionName.value());
    }
}
...
TEST_F(FileSystemConnectorTest, testFileCommit) {
    ...
    const std::vector<std::shared_ptr<FsWriterInfo>> writerInfos = fsDataSink->getWriteInfos();
    ASSERT_TRUE(writerInfos.size() == 2);
    auto fs_ = filesystems::getFileSystem(dataPath, fsConnector->connectorConfig());
    for (size_t i = 0; i < writerInfos.size(); ++i) {
        const std::shared_ptr<FsWriterInfo> writerInfo = writerInfos[i];
