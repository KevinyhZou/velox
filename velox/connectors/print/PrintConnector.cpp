...
std::unique_ptr<DataSink> PrintConnector::createDataSink(
    ...
    std::shared_ptr<PrintTableHandle> printTableHandle = std::dynamic_pointer_cast<PrintTableHandle>(connectorInsertTableHandle);;
    return std::make_unique<PrintSink>(inputType, printTableHandle->path(), connectorQueryCtx);
}
