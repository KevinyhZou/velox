...
const std::shared_ptr<StreamJSONDeserializer> StreamJSONDeserializer::create(
  ...
  if (kind == TypeKind::INTEGER) {
    ...
    return std::make_shared<StreamBoolDeserializer>();
  } else if (kind == TypeKind::DOUBLE) {
    return std::make_shared<StreamDoubleDeserializer>();
  }  else if (kind == TypeKind::REAL) {
    return std::make_shared<StreamFloatDeserializer>();
    ...
    auto elementDeserializer = create(arrayType->elementType());
    return std::make_shared<StreamArrayDeserializer>(
      arrayType->elementType(), elementDeserializer
    );
  } else if (kind == TypeKind::MAP) {
    const std::shared_ptr<const MapType> mapType =
      std::dynamic_pointer_cast<const MapType>(type);
    const auto keyDeserializer = create(mapType->keyType());
    const auto valueDeserializer = create(mapType->valueType());
    return std::make_shared<StreamMapDeserializer>(
