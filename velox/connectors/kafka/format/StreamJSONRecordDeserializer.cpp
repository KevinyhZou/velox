...
const std::shared_ptr<StreamJSONDeserializer> StreamJSONDeserializer::create(const TypePtr & type) {
    ...
    if (kind == TypeKind::INTEGER) {
        ...
        return std::make_shared<StreamRowDeserializer>(fieldNames, fieldTypes, deserializers);
    } else {
        VELOX_FAIL("The type is not supported: {}", type);
    }
}

const void KafkaStreamJSONRecordDeserializer::deserialize(const std::string & message, const size_t index, VectorPtr & vec) {
    if (!vec) {
        vec = BaseVector::create(outputType_, 1, memoryPool_);
    }
    try {
        simdjson::padded_string_view json_padded(message.data(), message.size(), message.size() + simdjson::SIMDJSON_PADDING);
