...
using HugeIntDeserializer = BaseDeserializer<int128_t>;
...
const std::shared_ptr<JSONDeserializer> JSONDeserializer::create(const TypePtr & type) {
    ...
    if (kind == TypeKind::INTEGER) {
        ...
        return std::make_shared<HugeIntDeserializer>();
        ...
        return std::make_shared<RowDeserializer>(fieldNames, fieldTypes, deserializers);
        ...
        const std::shared_ptr<JSONDeserializer> elementDeserializer = create(elementType);
        return std::make_shared<ArrayDeserializer>(elementType, elementDeserializer);
        ...
        const std::shared_ptr<JSONDeserializer> valueDeserializer = create(valueType);
        return std::make_shared<MapDeserializer>(keyType, valueType, keyDeserializer, valueDeserializer);
    } else {
        VELOX_FAIL("The type is not supported: {}", type);
    }
}

const void KafkaJSONRecordDeserializer::deserialize(const std::string & message, const size_t index, VectorPtr & vec) {
    try {
        Element element;
        ...
        LOG(WARNING) << "Failed to deserialize record: " << message <<" , error: " << e.what();
    }
}

const void KafkaJSONRecordDeserializer::deserialize(const std::vector<std::string> & messages, VectorPtr & vec) {
    Elements elements;
    std::vector<size_t> indexes;
    elements.reserve(messages.size());
    indexes.reserve(messages.size());
    for (size_t i = 0; i < messages.size(); ++i) {
        try {
            Element element;
            parser_->parse(messages[i].data(), messages[i].size()).get(element);
            elements.emplace_back(std::move(element));
            indexes.emplace_back(i);
        } catch (const std::exception & e) {
            LOG(WARNING) << "Failed to deserialize record: " << messages[i] <<" , error: " << e.what();
        }
    }
    deserializer_->deserialize(elements, indexes, vec);
}
