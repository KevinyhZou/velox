...
public:
    static const std::shared_ptr<StreamJSONDeserializer> create(const TypePtr & type);

    inline virtual const void deserialize(JSONValue & e, const size_t index, VectorPtr & vec) = 0;
};
...
public:
    ...
    inline const void deserialize(JSONValue & e, const size_t index, VectorPtr & vec) override {
        auto flat = std::dynamic_pointer_cast<FlatVector<facebook::velox::StringView>>(vec);
...
public:
    KafkaStreamJSONRecordDeserializer(const RowTypePtr & outputType, memory::MemoryPool * memoryPool) :
        KafkaRecordDeserializer(outputType, memoryPool),
        deserializer_(std::dynamic_pointer_cast<StreamRowDeserializer>(StreamJSONDeserializer::create(outputType))),
        parser_(std::make_shared<simdjson::ondemand::parser>()) {}

    const void deserialize(const std::string & message, const size_t index, VectorPtr & vec) override;
