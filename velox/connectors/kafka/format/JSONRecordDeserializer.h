...
public:
    virtual inline const void deserialize(const Element & e, const size_t index, VectorPtr & vec) = 0;
    virtual inline const void deserialize(const Elements & elements, const std::vector<size_t> & indexes, VectorPtr & vec) = 0;
...
public:
    inline const void deserialize(const Element & e, const size_t index, VectorPtr & vec) override {
    ...
    inline const void deserialize(const Elements & elements, const std::vector<size_t> & indexes, VectorPtr & vec) override {
        auto flat = std::dynamic_pointer_cast<FlatVector<T>>(vec);
        for (size_t i = 0; i < elements.size(); ++i) {
            if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>) {
                flat->set(indexes[i], static_cast<T>(elements[i].get_double().value()));
            } else if constexpr (std::is_same_v<T, bool>) {
                ...
                flat->set(indexes[i], timestamp);
            } else if constexpr (std::is_same_v<T, int8_t> 
                || std::is_same_v<T, int16_t> 
                || std::is_same_v<T, int32_t>
                || std::is_same_v<T, int64_t>
                || std::is_same_v<T, int128_t>){
                flat->set(indexes[i], static_cast<T>(elements[i].get_int64().value()));
            } else {
                VELOX_FAIL("The type {} is not supported", typeid(T).name());
            }
        }
...
public:
    RowDeserializer(
        ...
        fieldNames_(fieldNames), fieldTypes_(fieldTypes), deserializers_(deserializers) {}
    ...
    inline const void deserialize(const Elements & elements, const std::vector<size_t> & indexes, VectorPtr & vec) override {
        ...
        for (size_t i = 0; i < fieldNames_.size(); ++i) {
            ...
            fieldElements.reserve(elements.size());
            fieldIndexes.reserve(elements.size());
            for (size_t j = 0; j < elements.size(); ++j) {
                if (elements[j].is_null()) {
                    rowFields[i]->setNull(j, true);
                } else {
                    fieldElements.emplace_back(elements[j][fieldNames_[i]]);
                    fieldIndexes.emplace_back(j);
                }
            }
            deserializers_[i]->deserialize(fieldElements, fieldIndexes, rowFields[i]);
        }
    }
...
public:
    ArrayDeserializer(const TypePtr & elementType, const std::shared_ptr<JSONDeserializer> & deserializer) :
...
public:
    ...
    keyType_(keyType),valueType_(valueType), keyDeserializer_(keyDeserializer), valueDeserializer_(valueDeserializer) {}
...
public:
    KafkaJSONRecordDeserializer(const RowTypePtr & outputType, memory::MemoryPool * memoryPool) :
        KafkaRecordDeserializer(outputType, memoryPool),
        deserializer_(std::dynamic_pointer_cast<RowDeserializer>(JSONDeserializer::create(outputType))),
