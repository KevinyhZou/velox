...
String KafkaConnectorSplit::topicPartitonsToString(const cppkafka::TopicPartitionList & tps) {
    String s = "";
    if (tps.empty()) {
        return s;
    }
    for (size_t i = 0; i < tps.size() - 1; ++i) {
        s += "[" + tps[i].get_topic() + "," + std::to_string(tps[i].get_partition()) + "]";
        s += ",";
    }
    s += "[" + tps[tps.size() - 1].get_topic() + "," + std::to_string(tps[tps.size() - 1].get_partition()) + "]";
    return s;
}

String KafkaConnectorSplit::toString() const {
    return fmt::format(
...
std::shared_ptr<KafkaConnectorSplit> KafkaConnectorSplit::create(const folly::dynamic& obj) {
    VELOX_CHECK_EQ(obj["name"].asString(), 
        "KafkaConnectorSplit", 
        "Failed to create kafka connector split, as the object map name {} is not 'KafkaConnectorSplit'.",
        obj["name"].asString());
    std::unordered_map<String, std::vector<std::pair<uint32_t, int64_t>>> topics;
    if (obj["topicPartitions"].isArray()) {
        ...
        for (const auto & tp : tpObjs) {
            if (tp.isObject()) {
                ...
                std::pair<uint32_t, int64_t> p(static_cast<uint32_t>(partition.asInt()), static_cast<int64_t>(offset.asInt()));
                if (it != topics.end()) {
                    std::vector<std::pair<uint32_t, int64_t>> & partitionOffset = it->second;
